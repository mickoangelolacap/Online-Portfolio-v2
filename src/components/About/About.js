...
const About = ({ theme }) => {
  ...
  const HobbyLists = hobbies.map((hobby, index) => (
  ...
  const FrontendLang = Languages[0].FrontLanguages.map((language, index) => (
    <Language key={index} language={language} theme={theme} />
  ));
  const FullStackLang = Languages[0].FullLanguages.map((language, index) => (
    <Language key={index} language={language} theme={theme} />
  ));
  const OtherLang = Languages[0].OtherLanguages.map((language, index) => (
    <Language key={index} language={language} theme={theme} />
  ));
  ...
  return (
    <div id="about">
            ...
            <Col sm={7} className="aboutContent">
              ...
              <Row>
                <Col sm={6}>
                  ...
                  <p
                    className={`generalPara ${
                      theme === "dark" ? "opacityDark" : "opacityLight"
                    }`}
                  >
                    {" "}
                    <span>Responsiveness</span> is a must. It should work across
                    all devices. Also, It needs to be fast.
                  </p>
                </Col>
                <Col sm={6}>
                  ...
                  >
                    I always make sure that the design is for the people who
                    will use it. <span>Figma</span> is my main man.
                  </p>
                </Col>
              </Row>
            </Col>
      ...
      <Container>
        ...
        <ScrollAnimation animateIn="fadeInUp">
          <Row className="hobbies">{HobbyLists}</Row>
        </ScrollAnimation>
      </Container>
      <Container>
        <ScrollAnimation animateIn="fadeInUp">
          <Row>
            <Col sm={12}>
              <h1 className="secondaryTitle">
                Languages <span className="text-primary">i</span> speak.
              </h1>
            </Col>
            <Col md={4} xs={12}>
              <div>
